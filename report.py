#!/usr/bin/env python3
"""Build the Kingfisher Hollow biodiversity survey as branded static pages.

Each major survey view has its own document so a reader only parses the
content, charts, and photographs needed for the page they chose.  The pages are
styled to match kingfisher-hollow.com and composed like an editorial
infographic. Charts come from viz.py; this module owns the page shell,
...
(sync + build + Cloudflare Pages deploy). Generated public/ output is
gitignored and built by CI — never deploy it by hand."""
...
_PLOTLY_FRAGMENT_RE = re.compile(
    r'(?P<opening><div id="(?P<id>[^"]+)" class="plotly-graph-div" '
    r'data-plotly-chart(?P<attrs>[^>]*)>)</div>\s*'
    r'<script>(?P<script>[\s\S]*?window\.__plotlyRender\([\s\S]*?)</script>',
)


def _externalize_plotly_payloads(view_markup):
    """Move generated chart calls out of the HTML parser's initial workload.

    Plotly's HTML fragments include their complete data object in an inline
    script.  Rendering was already deferred, but those objects were still
    parsed and retained for every chart on page load.  Fingerprinted external
    scripts let the chart observer request one payload only when its card is
    near the viewport.
    """
    chart_dir = PUBLIC_DIR / "assets" / "charts"
    chart_dir.mkdir(parents=True, exist_ok=True)
    # This directory is exclusively generated by this function.  Removing old
    # fingerprints keeps a long-lived local build directory from accumulating
    # payloads that no current page can request.
    for old_payload in chart_dir.glob("*.js"):
        old_payload.unlink()
    payload_count = 0
    ...
    def replace(match):
        ...
        fingerprint = hashlib.sha256(script.encode("utf-8")).hexdigest()[:20]
        payload = chart_dir / f"{fingerprint}.js"
        payload.write_text(script, encoding="utf-8")
        payload_count += 1
        return (
            match.group("opening")[:-1]
            + f' data-plotly-src="/assets/charts/{payload.name}"></div>'
        )

    rewritten = _PLOTLY_FRAGMENT_RE.sub(replace, view_markup)
    if payload_count and "window.__plotlyRender(" in rewritten:
        raise RuntimeError("Failed to externalize every generated Plotly payload")
    print(f"[report-metrics] externalized-plotly-payloads: {payload_count}")
    return rewritten
...
# This is the single source of truth for routes, both tiers of navigation, and
# the previous/next trail at the end of each report section.  The order is the
# intended reading order, not merely a visual sort.
VIEW_CONFIG = {
    "all": {
        "label": "Overview",
        "route": "/",
        "description": "A living biodiversity survey of Kingfisher Hollow: recent records, discovery, survey effort, seasonality, and where observations are concentrated.",
        "plotly": True,
        "links": [
            ("#whats-new", "Latest"),
            ("#discovery", "Growth"),
    ...
    "life-list": {
        "label": "Life list",
        "route": "/life-list/",
        "description": "The complete Kingfisher Hollow life list: every documented species, searchable by name, taxon group, and family.",
        "plotly": False,
        "links": [("#life-list", "Full life list")],
    },
    "moths": {
        "label": "Moths",
        "route": "/moths/",
    ...
    "log": {
        "label": "Log",
        "route": "/log/",
        "description": "A night-by-night field journal of Kingfisher Hollow survey sessions, weather, observers, and newly documented species.",
        "plotly": False,
...
def _view_url(mode):
    """Absolute canonical URL for a configured survey page."""
    return f"https://survey.kingfisher-hollow.com{VIEW_CONFIG[mode]['route']}"


def _view_output_path(mode):
    """Static Pages output file for a configured survey route."""
    route = VIEW_CONFIG[mode]["route"].strip("/")
    return PUBLIC_DIR / route / "index.html" if route else PUBLIC_DIR / "index.html"


def _configured_legacy_hash_routes():
    """Return the durable top-level legacy links without generated markup."""
    routes = {
        href[1:]: config["route"]
    ...
    # Kept as an in-page alias in the moth report, but it was a historically
    # shared chapter link and now belongs on the moths page too.
    routes["moth-diversity"] = VIEW_CONFIG["moths"]["route"]
    return routes


def legacy_hash_redirect_script(routes=None):
    """Move pre-route deep links before the root document's body is parsed.

    URL fragments never reach Cloudflare Pages redirects, so this small root-only
    client redirect preserves old shared links such as /#log-journal without
    making a phone parse every survey view first.
    """
    routes = routes if routes is not None else _configured_legacy_hash_routes()
    return f"""<script>
(() => {{
  const routes={json.dumps(routes, separators=(',', ':'))};
  let id='';
  try {{ id=decodeURIComponent(location.hash.slice(1)); }} catch (_error) {{ return; }}
  const route=routes[id];
  if (route) location.replace(route+location.hash);
}})();
</script>"""
...
def log_taxa_total_pills(totals, recent=None):
    ...
    if recent is not None:
        ...
        if available:
            caption += (
                ' <span aria-hidden="true">·</span> + first documented in 30 days'
...
# ── head / nav / footer ──────────────────────────────────────────────────────
def head(s, county_firsts, moth_species=None, mode="all", legacy_routes=None):
    """Render a page-specific document head without loading unrelated views."""
    config = VIEW_CONFIG[mode]
    moth_phrase = f"{moth_species:,} moth species, " if moth_species else ""
    overview_desc = (f"Biodiversity survey of Kingfisher Hollow — {s['species']:,} species on 30 riparian acres along Michigan Creek, "
    ...
    desc = overview_desc if mode == "all" else config["description"]
    title = ("Kingfisher Hollow · Biodiversity Survey"
             if mode == "all"
             else f"Kingfisher Hollow · {config['label']} | Biodiversity Survey")
    canonical = _view_url(mode)
    plotly_queue = ("<script>\nwindow.__plotlyQueue=[];\n"
                    "window.__plotlyRender=(...args)=>{window.__plotlyQueue.push(args);window.dispatchEvent(new CustomEvent('kh-plotly-payload',{detail:args}));};\n"
                    "</script>" if config["plotly"] else "")
    legacy_redirect = legacy_hash_redirect_script(legacy_routes) if mode == "all" else ""
...
<body class="font-sans text-stone-800 antialiased" data-mode="{mode}">"""


def nav(active_mode="all"):
    """Render page navigation with ordinary links between static survey routes."""
    config = VIEW_CONFIG[active_mode]

    def mode_links(context):
        return "".join(
            f'<a class="mode-btn no-underline{" mode-active" if mode == active_mode else ""}" '
            f'href="{view_config["route"]}" data-context="{context}"'
            f'{" aria-current=\"page\"" if mode == active_mode else ""}>{view_config["label"]}</a>'
            for mode, view_config in VIEW_CONFIG.items())

    def section_links(mobile=False):
        items = []
        sequence = 0
        for href, label in config["links"]:
        ...
        return (
            f'<div class="links-{active_mode} {wrapper_class} is-active" '
            f'data-section-links="{active_mode}">{"".join(items)}</div>'
        )

    internal_links = [href for href, _label in config["links"] if href.startswith("#")]
    skip_link = internal_links[0] if internal_links else "#survey-content"
    desktop_links = section_links()
    mobile_links = section_links(mobile=True)
    section_count = len(internal_links)
    return f"""
<a id="skip-link" href="{skip_link}" class="sr-only focus:not-sr-only focus:fixed focus:z-[70] focus:top-2 focus:left-2 focus:bg-white focus:text-stone-900 focus:px-3 focus:py-2 focus:rounded-lg focus:ring-2 focus:ring-hollow-500">Skip to survey content</a>
    ...
    <div class="max-w-4xl mx-auto px-6 py-6">
      ...
      <div class="flex items-end justify-between gap-4 mb-3">
        <div>
          ...
          <p id="survey-menu-view-name" class="font-serif text-2xl text-white mt-1">{config["label"]}</p>
        </div>
        <span id="survey-menu-section-count" class="text-white/35 text-xs tabular-nums">{section_count} section{"" if section_count == 1 else "s"}</span>
      </div>
      {mobile_links}
...
PLOTLY_SCRIPT = """
  ...
  (function(){
    ...
    window.addEventListener('kh-plotly-payload',event=>receive(event.detail));
    function render(id){
    ...
    function loadPayload(chart){
      const src=chart.dataset.plotlySrc;
      if(!src||chart.dataset.plotlyPayloadLoading==='true'||queued.has(String(chart.id))) return;
      chart.dataset.plotlyPayloadLoading='true';
      const script=document.createElement('script');
      script.src=src;
      script.async=true;
      script.onload=()=>{chart.dataset.plotlyPayloadLoading='false';};
      script.onerror=()=>{
        chart.dataset.plotlyPayloadLoading='false';
        chart.dataset.plotlyPayloadFailed='true';
        chart.textContent='Chart data could not load.';
      };
      document.head.appendChild(script);
    }
...
__PLOTLY_SCRIPT__
  ...
  (function(){
    ...
    function applyHash(){
      ...
      if(section) setActiveSection(section.id,false);
    }
...
def _view_fragment(view_markup, mode):
    """Return one configured view wrapper from the generated view stream."""
    start_marker = f'<div id="view-{mode}"'
    start = view_markup.find(start_marker)
    if start < 0:
        raise RuntimeError(f"Missing generated view wrapper for {mode!r}")
    end = view_markup.find('<div id="view-', start + len(start_marker))
    return view_markup[start:] if end < 0 else view_markup[start:end]


def _legacy_hash_routes(view_markup):
    """Route every generated non-overview anchor from a former root URL.

    Fragments are never sent to Cloudflare Pages, so a Pages redirect cannot
    preserve ``/#reptiles-found`` or a similar historic link.  Build the map
    from each emitted view so newly added section anchors stay compatible.
    """
    routes = _configured_legacy_hash_routes()
    for mode, config in VIEW_CONFIG.items():
        if mode == "all":
            continue
        for anchor in re.findall(r'\bid="([^"]+)"', _view_fragment(view_markup, mode)):
            routes[anchor] = config["route"]
    return routes


def _write_survey_pages(view_markup, public_summary, county_firsts, moth_species,
                        code_updated, content_updated, data_updated):
    """Write one static document per configured route and return their digests."""
    digests = {}
    legacy_routes = _legacy_hash_routes(view_markup)
    for mode in VIEW_CONFIG:
        out = _view_output_path(mode)
        out.parent.mkdir(parents=True, exist_ok=True)
        html = "".join((
            head(public_summary, county_firsts, moth_species, mode=mode,
...
def build():
    ...
    # The full life list is by far the largest page section.  Keep the complete
    # searchable table, but put it on its own document instead of making every
    # overview visit parse and lay it out.
    parts.append('<div id="view-life-list" data-view-label="Life list">')
    parts.append(life_list_section)
    parts.append('</div>')  # /view-life-list

    # ── Moths view (dark) ────────────────────────────────────────────────────
    parts.append('<div id="view-moths" data-view-label="Moths">')
    parts.append(_timed("moth-view", moth_view, df, stats))
    parts.append('</div>')  # /view-moths

    # ── Butterflies view (dark) ──────────────────────────────────────────────
    parts.append('<div id="view-butterflies" data-view-label="Butterflies">')
    ...
    page_digests = _write_survey_pages(
        _externalize_plotly_payloads("".join(parts)),
        public_s,
        county_firsts,
        moth_head_count,
        _code_updated(),
    ...
    for out, digest in page_digests.items():
        if hashlib.sha256(out.read_bytes()).hexdigest() != digest:
            raise RuntimeError(f"Field guide build modified generated survey page {out}")
    _log_timing("report-total", build_start)
    return _view_output_path("all")
